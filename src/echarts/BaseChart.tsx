...
const EChart = (props: IBaseChartProps, ref: any) => {
  const { options, loading = false, onClick, style = {} } = props
  const cDom = useRef(null)
  const cInstance: any = useRef(null)
  ...
  useEffect(() => {
    if (cDom.current) {
      ...
      // 绑定鼠标点击事件
      cInstance.current.on('click', (event: any) => {
  ...
  return <div id='eCharts' ref={cDom} style={{ width: '100%', height: '100%', ...style }} />
}
